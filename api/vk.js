...
async function createModerationAction(peerId, actorVkId, actionType, targetInput, durationText, reason = '') {
  ...
  if (actionType === 'ban' && boolEnv('VK_AUTO_KICK_ON_BAN', false) && isGroupPeer(peerId)) {
    const chatId = Number(peerId) - 2000000000;
    await vkApi('messages.removeChatUser', { chat_id: String(chatId), user_id: String(targetVkId) }).catch(() => null);
  }
...
function helpText(vkUserId, peerId) {
  ...
  return [
    ...
    '',
    '👑 Владелец',
    '/group type staff — сюда заявки',
    '/group type reports — сюда отчёты',
