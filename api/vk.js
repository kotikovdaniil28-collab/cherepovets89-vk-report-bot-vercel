...
async function activeStickyBanFor(peerId, targetVkId) {
  ...
  try {
    const { data, error } = await getSupabase()
      ...
      .select('id,reason,created_at,actor_vk_user_id')
      .eq('peer_id', String(peerId))
...
async function unbanVkUser(peerId, actorVkId, targetInput) {
  if (!(await canUseModActions(actorVkId))) {
    await sendMessage(peerId, '⛔ Недостаточно прав.');
    return;
  }
  const targetVkId = await resolveVkTarget(targetInput);
  if (!targetVkId) {
    await sendMessage(peerId, '⚠️ Не понял пользователя. Пример: /анбан @id123');
    return;
  }
...
async function cancelModerationAction(peerId, actorVkId, actionId) {
  ...
  if (targetVkId) {
    const cancelled = await cancelActiveModerationActions(peerId, actorVkId, { targetVkId });
    await sendMessage(peerId, [
      '🧹 Снятие наказаний',
      `👤 VK: ${targetVkId}`,
      cancelled
        ? `✅ Активных наказаний снято: ${cancelled}`
        : 'ℹ️ Активных наказаний в этой беседе не найдено.',
...
async function helpText(vkUserId, peerId, pageInput = '') {
  ...
  const pages = {
    ...
    punish: [
      ...
      '• /анмут @id123 / /размут @id123',
      '• /бан @id123 7д реклама',
      '• /анбан @id123 / /разбан @id123',
      '• /пред @id123 причина',
      ...
      '• /снятьнаказание @id123 — снять все активные наказания в беседе',
      '',
...
async function handleMessageNew(payload) {
  ...
  if (
    reportsStrictModeEnabled() &&
    (await isReportPeer(peerId).catch(() => false)) &&
    !session &&
    !REPORT_COMMAND_RE.test(text) &&
    !text.startsWith('/')
  ) {
    await deleteMessagesBestEffort(peerId, [messageId(message)]);
    return;
