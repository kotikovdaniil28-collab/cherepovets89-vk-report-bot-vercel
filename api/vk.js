...
async function fetchPendingGoogleSheetApplications(limit = 5) {
  ...
  const u = new URL(url);
  u.searchParams.set('mode', 'pending');
  u.searchParams.set('limit', String(Math.min(Math.max(Number(limit) || 5, 1), 25)));
...
function formatGooglePendingApplication(item) {
  ...
  const verdict = Array.isArray(item.lastTwoValues) ? item.lastTwoValues.map(cleanText).filter(Boolean).join(' | ') : '';
...
function helpText(vkUserId, peerId) {
  const groupLine = reportsPeerId()
    ...
    : 'Группа отчётов: через /group type reports';
  ...
  return [
    ...
    '/отмена — отменить форму',
    '',
    '🛡 Модерация',
    '/отчёты — на проверке',
    '/отчёты все 10 — все отчёты',
    '/отчёты email mail@example.com',
    '/репорт <id> — детали',
    '/принять <id> [xp]',
    '/отклонить <id> [причина]',
    '/заявки [число] — строки Google Sheet без вердикта',
    '/модеры · /роли',
    '/юзер @id123 / @shortname / email',
    '/стата @id123',
    '/найти <email/ник>',
    '/мут @id123 90м причина',
    '/размут @id123',
    ...
    '',
    '👑 Владелец',
    '/group type staff — staff-чат',
    '/group type reports — чат отчётов',
    '/groups · /group info · /group clear',
    '/роль @id123 Куратор|КМ|ЗГМ|Модератор',
    '/роль снять @id123',
    '/модер выдать @id123',
