...
## v6: если заявки из Google Forms не приходят
1. Открой в браузере `https://твой-проект.vercel.app/api/google-sheet-webhook?secret=ТВОЙ_СЕКРЕТ`.
   В ответе `staffPeerId` должен быть не `null`.
2. В staff-беседе VK напиши `/group type staff` и после этого проверь `/group info`.
3. В Apps Script запусти `testWebhookOnly()`. Если будет ошибка — смотри `Execution log`.
4. Лист должен называться точно `Ответы на формы (3)`, либо поменяй `GOOGLE_SHEET_TARGET_NAME` в Vercel и `CH89_TARGET_SHEET_NAME` в Apps Script.

## v6: реальные VK-действия
`/мут @id123 10м причина` теперь пытается вызвать VK `messages.changeConversationMemberRestrictions` и запретить писать в беседе.
`/размут @id123` снимает ограничение через тот же метод.
`/бан @id123 7д причина` по умолчанию пытается удалить пользователя из беседы через `messages.removeChatUser`.
Если VK не даст доступ к методу, бот покажет причину в ответе, а запись наказания всё равно сохранит в Supabase.
