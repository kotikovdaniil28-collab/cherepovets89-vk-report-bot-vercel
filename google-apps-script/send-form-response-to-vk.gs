...
function fillStaffRow_(row) {
  ...
  return {
    ok: true,
      service: 'ch89-staff-sheet-fill-v24-candidates-group',
    sheetName: sheet.getName(),
    rowNumber,
    spreadsheetUrl: SpreadsheetApp.getActiveSpreadsheet().getUrl(),
  };
...
function repairStaffRow_(rowNumber) {
  ...
  return {
    ok: true,
    service: 'ch89-staff-sheet-repair-v24-candidates-group',
    sheetName: sheet.getName(),
    rowNumber: n,
    fixed: {
      vk: vkUrl || '',
...
function setApplicationVerdict_(sheet, rowNumber, verdict, reason, actor) {
  ...
  return {
    ok: true,
    service: 'ch89-google-sheets-verdict-v24',
    sheetName: sheet.getName(),
    rowNumber: n,
...
function doGet(e) {
  ...
  if (mode === 'staff_debug' || mode === 'staff_test') {
    ...
    return json_({
      ok: true,
      service: 'ch89-staff-sheet-v24-candidates-group',
      staffFill: true,
  ...
  if (mode === 'debug' || mode === 'gsheet' || mode === 'diag') {
    return json_({ ok: true, service: 'ch89-google-sheets-v24-debug', ...debugInfo_(sheet) });
  }
  ...
  return json_({
    ok: true,
    service: 'ch89-google-sheets-pending-v24',
    sheetName: sheet.getName(),
    limit,
