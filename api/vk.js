...
async function reportPeerHelpText(peerId) {
  ...
  return [
    '⛔ Отчёты принимаются только в группе отчётов.',
    `💬 Текущий peer_id: ${peerId}`,
    `🏷 Тип текущей группы: ${groupTypeTitle(type || 'off')}`,
    configured ? `📌 REPORTS_PEER_ID: ${configured}` : '',
    '',
    'Чтобы сдавать отчёты здесь, владелец может написать в этой беседе:',
    '/group type reports',
  ].filter(Boolean).join('\n');
}
...
async function googleSheetDebugCommand(peerId) {
  try {
    ...
    const data = await fetchGoogleSheetData('debug', 1);
    if (!data) {
...
async function createModerationAction(peerId, actorVkId, actionType, targetInput, durationText, reason = '') {
  if (!(await canUseModActions(actorVkId))) {
    await sendMessage(peerId, '⛔ Модерские действия доступны ролям: Модератор, КМ, Куратор, ЗГМ, ГМ.');
    return;
  }
  const targetVkId = await resolveVkTarget(targetInput);
  if (!targetVkId) {
...
async function handleModCommand(peerId, vkUserId, text) {
  ...
  if (/^\/(?:роли|roles|staff)$/i.test(raw)) {
    if (!(await canUseModActions(vkUserId))) {
  ...
  const roleSet = raw.match(/^\/(?:роль|role|права)\s+(.+?)\s+(гм|gm|згм|zgm|куратор|curator|км|km|модер|модератор|mod)(?:\s+([\s\S]+))?$/i);
  if (roleSet) {
    await grantVkStaffRole(peerId, vkUserId, roleSet[1], roleSet[2], roleSet[3] || '');
    return true;
  }

  const roleClear = raw.match(/^\/(?:роль|role|права)\s+(?:снять|remove|del|delete)\s+(.+)$/i);
  if (roleClear) {
    await revokeVkStaffRole(peerId, vkUserId, roleClear[1]);
    return true;
  }

  const userAny = raw.match(/^\/(?:юзер|user|профиль)\s+(.+)$/i);
  ...
  const statsAny = raw.match(/^\/(?:стата|stats|статистика)\s+(.+)$/i);
  ...
  const unmute = raw.match(/^\/(?:размут|unmute)\s+(.+)$/i);
  if (unmute) {
    await unmuteVkUser(peerId, vkUserId, unmute[1]);
    return true;
  }

  const punishList = raw.match(/^\/(?:наказания|punishments)\s+(.+)$/i);
  if (punishList) {
    await listModerationActions(peerId, vkUserId, punishList[1]);
    return true;
  }

  const punishCancel = raw.match(/^\/(?:снятьнаказание|unpunish)\s+(\S+)$/i);
  if (punishCancel) {
  ...
  const reportsAll = raw.match(/^\/(?:отчеты|отчёты|reports)\s+(?:все|all)(?:\s+(\d{1,2}))?$/i);
  if (reportsAll) {
    if (!(await canUseStaffCommands(vkUserId, peerId))) {
  ...
  const reportsByEmail = raw.match(/^\/(?:отчеты|отчёты|reports)\s+(?:email|почта)\s+([^\s]+)(?:\s+(\d{1,2}))?$/i);
  if (reportsByEmail) {
    if (!(await canUseStaffCommands(vkUserId, peerId))) {
      await sendMessage(peerId, '⛔ Просмотр отчётов доступен владельцу или модератору.');
      return true;
  ...
  const reportsStatus = raw.match(/^\/(?:отчеты|отчёты|reports)\s+(?:статус|status)\s+([^|]+?)(?:\s+(\d{1,2}))?$/i);
  if (reportsStatus) {
    if (!(await canUseStaffCommands(vkUserId, peerId))) {
      await sendMessage(peerId, '⛔ Просмотр отчётов доступен владельцу или модератору.');
      return true;
  ...
  const search = raw.match(/^\/(?:найти|search|users)\s+(.+)$/i);
  if (search) {
    if (!(await canUseStaffCommands(vkUserId, peerId))) {
  ...
  const grantByEmail = raw.match(/^\/(?:модер|mod)\s+(?:выдать|дать|add)\s+(?:email|почта)\s+([^\s]+)$/i);
  if (grantByEmail) {
    await grantModeratorByEmail(peerId, vkUserId, grantByEmail[1]);
    return true;
  }

  const revokeByEmail = raw.match(/^\/(?:модер|mod)\s+(?:снять|remove|del|delete)\s+(?:email|почта)\s+([^\s]+)$/i);
  if (revokeByEmail) {
    await revokeModeratorByEmail(peerId, vkUserId, revokeByEmail[1]);
    return true;
  }

  const grantAny = raw.match(/^\/(?:модер|mod)\s+(?:выдать|дать|add)\s+(\S+)$/i);
  ...
  const revokeAny = raw.match(/^\/(?:модер|mod)\s+(?:снять|remove|del|delete)\s+(\S+)$/i);
  ...
  const linkEmail = raw.match(/^\/(?:привязать|link)\s+(?:email|почта)\s+(\d{2,20})\s+([^\s]+)(?:\s+(.+))?$/i);
  if (linkEmail) {
    await linkByEmailCommand(peerId, vkUserId, linkEmail[1], linkEmail[2], linkEmail[3] || '');
    return true;
  }

  const userEmail = raw.match(/^\/(?:юзер|user|профиль)\s+(?:email|почта)\s+([^\s]+)$/i);
  if (userEmail) {
  ...
  if (/^\/(?:gsheet|гугл|таблица)$/i.test(raw)) {
    if (!(await canUseStaffCommands(vkUserId, peerId))) {
...
async function helpText(vkUserId, peerId) {
  const role = await getVkStaffRole(vkUserId).catch(() => '');
  const groupType = await getGroupType(peerId).catch(() => '');
  const configured = reportsPeerId();
  const groupLine = [
    `💬 peer_id: ${peerId}`,
    `🏷 Группа: ${groupType ? groupTypeTitle(groupType) : 'не задана'}`,
    configured ? `🧾 REPORTS_PEER_ID: ${configured}${String(peerId) === String(configured) ? ' ✅' : ''}` : '🧾 Чат отчётов: /group type reports',
  ...
  return [
    ...
    '',
    '👤 ОБЩИЕ',
    '• /help, /помощь, /команды — это меню',
    '• /id, /ид, /айди, /vkid — ваш VK ID и peer_id чата',
    '• /ping, /пинг — проверка ответа бота',
    '• /правило 2.1 или /правило флуд — поиск по правилам',
    '• /термин мут — объяснение термина',
    '',
    '🧾 ОТЧЁТЫ МОДЕРАТОРА',
    '• /отчет, /отчёт, /сдать, /сдача — открыть форму отчёта',
    `• /отчет работа | ${moscowDateIso()} | Норма | ссылка — быстрая сдача`,
    '• /отчет работа | Норма | ссылка — быстрая сдача за сегодня',
    '• /отмена — отменить заполнение формы',
    'Типы: Норма, Перенорма, Натяг, Герой дня.',
    '',
    '🛡 МОДЕРАТОР / КМ',
    '• /отчёты [5] — отчёты на проверке',
    '• /отчёты все 10 — последние отчёты',
    '• /отчёты email mail@example.com — отчёты пользователя',
    '• /репорт <id> — карточка отчёта',
    '• /принять <id> [xp] / /отклонить <id> [причина]',
    '• /юзер @id123 / ник / email — профиль',
    '• /стата @id123 / ник — статистика',
    '• /найти <email/ник> — поиск пользователей',
    '',
    '⚖️ НАКАЗАНИЯ: МОДЕРАТОР / КМ / ЗГМ',
    '• /мут @id123 90м причина — мут',
    '• /размут @id123 — снять мут',
    '• /бан @id123 7д причина — бан/кик из беседы',
    '• /пред @id123 причина — предупреждение',
    '• /устник @id123 причина — устное предупреждение',
    '• /строгий @id123 причина — строгое предупреждение',
    '• /наказания @id123 — история наказаний',
    'Сроки: 30м, 2ч, 1д. Алиасы: /mute, /ban, /warn.',
    '',
    '👑 ЗГМ / ГМ / ВЛАДЕЛЕЦ',
    '• /роль @id123 Модератор|КМ|Куратор|ЗГМ — выдать VK-роль',
    '• /роль снять @id123 — снять VK-роль',
    '• /роли — список staff-ролей',
    '• /модер выдать @id123 или email mail@example.com — выдать модератора сайта',
    '• /модер снять @id123 или email mail@example.com — снять модератора сайта',
    '• /привязать email <vk_id> <email> [ник] — привязать VK к сайту',
    '',
    '🔧 ТОЛЬКО ВЛАДЕЛЕЦ',
    '• /group type reports|staff|ai|general — назначить тип беседы',
    '  Алиасы: /группа тип отчеты|стафф|ии|общая|выкл',
    '• /group info, /groups, /group clear — управление беседами',
    '• /xp @id123 +100 причина — изменить XP',
    '• /заявки [5] — заявки из Google Sheet',
    '• /gsheet — диагностика Google Sheet',
    '',
    '🧠 AI',
    '• /совет <ситуация> / /разбор <кейс> / /наказание <нарушение>',
    '• /шаблон <ответ> или бот, <вопрос>',
  ].join('\n');
}
async function handleMessageNew(payload) {
  ...
  if (/^\/(?:id|ид|айди|vkid|вкид)$/i.test(text)) {
    await sendMessage(peerId, `🆔 Ваш VK ID: ${vkUserId}\n💬 peer_id этого чата: ${peerId}`);
    return;
  }

  if (/^\/(?:help|хелп|помощь|commands|команды|start|старт)$/i.test(text)) {
    await sendMessage(peerId, await helpText(vkUserId, peerId));
    return;
  }
