...
- Все существующие inline-кнопки бота переведены с `text` на `callback`.
- Нажатие больше не публикует команду пользователя в беседе.
- Разделы `/help` переключаются внутри одной карточки.
- Обновлены подписи, структура главного меню и STAFF-панели.
- Callback-диспетчер поддерживает заявки, историю наказаний, отмену наказания, панель и навигацию.

## Что добавлено в v48

- Скриншот модератора прикрепляется к итоговой карточке отчёта в VK.
- В карточке есть callback-кнопки `Одобрить` и `Отказать`.
- Нажатия не создают сообщений-команд в беседе: меню открывается внутри той же карточки.
- При одобрении руководство выбирает итоговый тип: Норма, Перенорма, Натяг или Герой дня.
- При отказе открывается меню готовых причин.
- После решения бот пишет модератору в ЛС, если VK привязан к сайту.
- Добавлена карьерная лестница ММ → М → СМ и ручное решение о повышении через STAFF.
- Обычный путь ММ → М: 7 дней и 7 одобренных отчётов; досрочный: 5 дней и 5 Перенорм/Героев дня.
- Обычный путь М → СМ: 14 дней и 14 одобренных отчётов; досрочный: 10 дней и 10 Перенорм/Героев дня.
- Старый лист `Discord состав` синхронизируется с сайтом и ботом без второго Apps Script.
- Должности `ГМ`, `ЗГМ`, `КМ`, `СМ`, `М`, `ММ` берутся из столбца B; VK — из E; даты — из L/N.
- `ГМ`, `ЗГМ` и `КМ` получают на сайте роль руководства, остальные — роль модератора и реальную карьерную ступень.

## Обязательное обновление

1. Выполните `supabase/latest.sql` в текущем Supabase-проекте.
2. В существующем Apps Script замените код содержимым `google-apps-script/send-form-response-to-vk.gs`. Файл содержит весь старый функционал и новый блок `CH89 V48`; отдельный Web App не нужен.
3. В Apps Script выберите `Deploy → Manage deployments → Edit → New version → Deploy`. URL оставьте прежним.
4. В Callback API сообщества VK включите событие `message_event`/«Событие действия с сообщением».
5. Добавьте `CRON_SECRET` в переменные Vercel бота. Ежедневная проверка запускается в 10:00 МСК.
6. После деплоя выполните в STAFF: `/состав синхронизировать`, затем `/повышения проверить`.

```text
/состав синхронизировать
/повышения проверить
```

Если у строки нет рабочей VK-ссылки или VK ещё не привязан к сайту, бот не угадывает пользователя и показывает его в списке «Требуют внимания».

## Что исправлено в v47

- Исправлены формулы Google Sheets для русской локали: убраны пробелы вокруг `;`.
- Гиперссылки теперь создаются так:

```text
=ГИПЕРССЫЛКА("https://vk.com/id400266941";"VK ↗")
```

- Формулы дней теперь создаются так:

```text
=ЕСЛИ(L14="";"";СЕГОДНЯ()-L14)
```

Build для проверки: `v47-sheet-formula-fix`.
