...
async function postStaffSheetRow(row) {
  ...
  if (!url) throw new Error('GOOGLE_APPS_SCRIPT_URL не задан в Vercel');
  ...
  try {
    ...
    data = {
      ...
      error: /^<!doctype|<html/i.test(compact)
        ? 'Apps Script вернул HTML-страницу ошибки. Обнови Web App deployment: новая версия → развернуть → доступ Everyone/Все.'
        : compact,
    };
  }
  if (!response.ok || !data.ok) {
    ...
    if (/unknown mode/i.test(details)) {
      throw new Error(
        'Apps Script старой версии: он не знает mode=staff_fill. ' +
        'Открой Apps Script, вставь новый send-form-response-to-vk.gs, затем Управление развертываниями → Изменить → Версия: новая → Развернуть.'
      );
    }
...
async function testStaffSheetIntegration(peerId) {
  ...
  if (!response.ok || !data.ok) {
    ...
    await sendMessage(peerId, [
      '⚠️ Apps Script не готов для автозаполнения состава.',
      /unknown mode/i.test(details)
        ? 'Причина: опубликована старая версия скрипта без staff_fill.'
        : `Причина: ${escapeLine(details)}`,
      '',
      'Что сделать:',
      'Apps Script → Управление развертываниями → Изменить → Версия: новая → Развернуть.',
    ].join('\n'));
    return;
  }

  await sendMessage(peerId, [
    '✅ Apps Script готов для автозаполнения состава',
    `📄 Лист: ${escapeLine(data.staffSheetName || 'Discord состав')}`,
    `🔧 Service: ${escapeLine(data.service || '—')}`,
  ].join('\n'));
}
...
async function repairStaffSheetRow(peerId, rowNumber) {
  ...
  if (!response.ok || !data.ok) {
    ...
    await sendMessage(peerId, [
      ...
      /unknown mode/i.test(details)
        ? 'Причина: опубликована старая версия Apps Script без staff_fix.'
        : `Причина: ${escapeLine(details)}`,
      'Обнови Apps Script: новая версия → развернуть.',
    ].join('\n'));
    return;
  }
...
async function ensureStaffGroupCommand(peerId, vkUserId) {
  ...
  if (groupType !== 'staff') {
    await sendMessage(peerId, [
      ...
      `🏷 Текущий тип: ${groupType ? groupTypeTitle(groupType) : 'не задан'}`,
      'ГМ может назначить эту беседу:',
      '/group type staff',
    ].join('\n'));
    return false;
  }
...
async function addStaffSheetRowCommand(peerId, vkUserId, text) {
  ...
  await sendMessage(peerId, [
    '✅ Состав заполнен в Google Sheet',
...
async function addAcceptedCandidateToGroup(result) {
  ...
  if (!candidatesPeerId) {
    return { ok: false, status: 'not_configured', text: 'группа кандидатов не задана: /group type candidates' };
  }
  ...
  if (targetVkId && chatId) {
    ...
    addError = targetVkId ? 'peer_id кандидатов не является беседой' : 'VK кандидата не найден в анкете';
  }
...
async function listApplicationDecisionLog(peerId, vkUserId, limit = 10) {
  ...
  if (error) {
    await sendMessage(peerId, `⚠️ Лог заявок недоступен: ${escapeLine(error.message || error)}\nВыполни SQL: supabase/v23_application_decisions.sql`);
    return;
  }
  if (!data || !data.length) {
...
async function applicationVerdictCommand(peerId, vkUserId, action, rowNumber, reason = '', options = {}) {
  ...
  try {
    ...
    logWarning = `\n⚠️ Лог не сохранился: ${escapeLine(error.message || error)}\nВыполни SQL v23.`;
  }
  ...
  if (verdict === 'Принят') {
    ...
    candidateLine = candidateResult.ok
      ? `👥 Кандидат добавлен в группу кандидатов: @id${candidateResult.targetVkId}`
      : `👥 Группа кандидатов: ${escapeLine(candidateResult.text || candidateResult.error || 'не удалось добавить')}`;
...
async function handleGroupCommand(peerId, vkUserId, text) {
  ...
  if (type) {
    ...
    await sendMessage(peerId, [
      ...
      normalized === 'candidates' ? '👥 Принятых кандидатов бот будет пытаться добавлять сюда.' : '',
...
async function listModerationLog(peerId, actorVkId, limitInput = 15) {
  ...
  const lines = data.map(row => {
    ...
    return [
      ...
      `  👮 ${row.actor_vk_user_id} · 💬 ${row.peer_id || '—'}`,
      row.duration_text ? `  ⏱ ${row.duration_text}${expiresAt ? ` · до ${expiresAt.toLocaleString('ru-RU', { timeZone: 'Europe/Moscow' })}` : ''}` : '',
      row.reason ? `  💭 ${escapeLine(row.reason)}` : '',
      row.status !== 'active' ? `  📌 ${row.status}${row.cancelled_by_vk_user_id ? ` · снял ${row.cancelled_by_vk_user_id}` : ''}` : '',
    ].filter(Boolean).join('\n');
  });
...
function countText(value) {
  return value == null ? 'ошибка' : String(value);
}
...
async function panelCommand(peerId, vkUserId) {
  ...
  await sendMessage(peerId, [
    ...
    `• VK-привязок: ${countText(linkedCount)}`,
    `• Привязанных бесед: ${countText(groupsCount)}`,
    `• Ошибок Google: ${countText(googleErrors)}`,
    '',
    'Быстрые команды:',
    '• /help — разделы команд',
    '• /health — диагностика',
...
async function healthCommand(peerId, vkUserId) {
  ...
  const checks = [];
  const add = (name, ok, detail = '') => checks.push(`${ok ? '✅' : '⚠️'} ${name}: ${detail || (ok ? 'OK' : 'нет')}`);

  add('VK_GROUP_TOKEN', !!env('VK_GROUP_TOKEN'));
  add('SUPABASE_URL', !!env('SUPABASE_URL'));
  add('SUPABASE_SERVICE_ROLE_KEY', !!env('SUPABASE_SERVICE_ROLE_KEY'));
  add('OWNER_VK_ID', !!ownerVkId(), ownerVkId() || 'нет');
  add('GOOGLE_APPS_SCRIPT_URL', !!googleSheetPullUrl());
  add('GOOGLE_SHEET_PULL_SECRET', !!googleSheetPullSecret());
  add('DEEPSEEK_API_KEY', !!env('DEEPSEEK_API_KEY'));

  try {
    await getSupabase().from('vk_links').select('vk_user_id', { head: true, count: 'exact' });
    add('Supabase vk_links', true);
  } catch (error) {
    add('Supabase vk_links', false, escapeLine(error.message || error));
  }

  try {
    await getSupabase().from('vk_link_codes').select('code', { head: true, count: 'exact' });
    add('Supabase vk_link_codes', true);
  } catch (error) {
    add('Supabase vk_link_codes', false, 'выполни SQL v17');
  }
  ...
  try {
    ...
    add('Текущая беседа', false, escapeLine(error.message || error));
  }

  if (googleSheetPullUrl()) {
    try {
      const data = await fetchPendingGoogleSheetApplications(1);
      add('Google Apps Script', !!data.ok, data.staffSheetName || data.sheetName || 'OK');
    } catch (error) {
      add('Google Apps Script', false, escapeLine(error.message || error));
    }
  }
  ...
  await sendMessage(peerId, [
    ...
    ...checks,
    '',
    'Если vk_link_codes жёлтый — выполни новый SQL из архива.',
  ].join('\n'), { keyboard: helpKeyboard('main') });
}
...
async function linkVkByCodeCommand(peerId, vkUserId, codeInput) {
  ...
  if (error) {
    await sendMessage(peerId, `❌ Ошибка привязки: ${escapeLine(error.message)}\nВыполни SQL v17, если таблицы vk_link_codes ещё нет.`);
    return;
  }
...
async function helpText(vkUserId, peerId, pageInput = '') {
  ...
  const pages = {
    ...
    gm: [
      ...
      '• /health — проверка VK/Supabase/Google/DeepSeek',
    ...
    apps: [
      ...
      '',
      'Бот ищет колонку “Вердикт”. Если её нет, Apps Script создаст “Вердикт” и “Комментарий”.',
      'Открытая заявка: пусто, “На рассмотрении”, “Ожидает”, pending.',
      'Для автосообщений нужна staff-беседа: /group type staff',
      'Для автодобавления принятых нужна беседа кандидатов: /group type candidates',
    ],
    staffsheet: [
      ...header,
      '',
...
function isReportsAllowedPublicCommand(text) {
  ...
  return /^\/(?:отмена|cancel|stop|ид|id|айди|vkid|вкид|peer|пир|help|хелп|помощь|commands|команды|start|старт|ping|пинг)(?:\s|$)/i.test(raw);
}
  ...
  if (req.method === 'GET') {
    ...
    if (['expire', 'expire_moderation', 'punishments'].includes(task)) {
      ...
      try {
        ...
        res.status(200).json({ ok: true, service: 'cherepovets-vk-bot-v25-expire-task', expired });
      } catch (error) {
    ...
    res.status(200).json({ ok: true, service: 'cherepovets-vk-bot-v25-application-to-staff', reportsPeerId: reportsPeerId() || null });
    return;
  }
