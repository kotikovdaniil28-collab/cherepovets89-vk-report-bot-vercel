...
async function generateGeminiImage(prompt) {
  ...
  const body = {
    contents: [{
      role: 'user',
      parts: [{ text: `Сгенерируй изображение. Без текста на картинке, если это не просят явно.\n\nОписание: ${prompt}` }],
    }],
    generationConfig: {
  ...
  const image = geminiInlineImage(data);
  if (!image?.data) throw new Error('Gemini не вернул изображение. Проверь модель GEMINI_IMAGE_MODEL.');
  return {
...
async function handleImageCommand(peerId, vkUserId, text) {
  ...
  try {
    ...
    imageUrl = await uploadGeneratedImageToStorage(vkUserId, image.buffer, image.contentType).catch(() => '');
...
async function askDeepSeek(mode, question, context = {}) {
  ...
  if (!apiKey) {
    return 'AI-помощник пока не подключён.';
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (/invalid|authentication|api key|unauthorized/i.test(details)) {
        return 'AI-помощник сейчас недоступен.';
      }
      return `AI-помощник временно недоступен: ${userFacingError(details)}`;
    }

    const answer = data?.choices?.[0]?.message?.content || '';
    return compactAiAnswer(answer) || 'AI-помощник не нашёл короткий ответ.';
  } catch (error) {
    if (error.name === 'AbortError') return 'AI-помощник не успел ответить. Сократи запрос.';
    return `AI-помощник временно недоступен: ${userFacingError(error)}`;
  } finally {
...
async function shouldAtmosphereMessage(peerId, vkUserId, text) {
  ...
  const type = await getGroupType(peerId).catch(() => '');
  if (!['ai', 'staff', 'candidates'].includes(type)) return false;
  if (await isNoModerationGroup(peerId).catch(() => false)) return false;
