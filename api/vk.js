...
function parseStaffSheetPayload(text) {
  ...
  const vkUrl = firstUrlByPattern(joined, /vk\.com/i) || (joined.match(/(?:^|\s)@?id\d{2,20}(?:\s|$)/i)?.[0] || '');
...
async function postStaffSheetRow(row) {
  ...
  const request = {
    ...
    body: JSON.stringify({ mode: 'staff_fill', secret, row }),
  };

  let response = await fetch(u.toString(), request);
  if ([301, 302, 303, 307, 308].includes(response.status)) {
    const location = response.headers.get('location');
    if (location) response = await fetch(location, { ...request, redirect: 'follow' });
  }

  const text = await response.text();
  let data;
  try { data = JSON.parse(text || '{}'); } catch (_) { data = { ok: false, error: text.slice(0, 500) }; }
  if (!response.ok || !data.ok) throw new Error(data.error || data.message || `Apps Script HTTP ${response.status}`);
  return data;
}
