  ...
  if (req.method === 'GET') {
    ...
    if (['expire', 'expire_moderation', 'punishments'].includes(task)) {
      ...
      try {
        ...
        res.status(200).json({ ok: true, service: 'cherepovets-vk-bot-v24-expire-task', expired });
      } catch (error) {
    ...
    res.status(200).json({ ok: true, service: 'cherepovets-vk-bot-v24-candidates-group', reportsPeerId: reportsPeerId() || null });
    return;
  }
