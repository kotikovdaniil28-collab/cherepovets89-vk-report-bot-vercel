...
function normalizeGroupType(value) {
  ...
  return new Map([
    ...
    ['link', 'link'], ['связь', 'link'], ['гагм', 'link'], ['га-гм', 'link'], ['связьгагм', 'link'], ['svyaz', 'link'],
    ['off', 'off'], ['выкл', 'off'], ['снять', 'off'], ['нет', 'off'],
  ]).get(raw) || '';
}

function allowedGroupTypes() {
  return new Set(['reports', 'staff', 'candidates', 'general', 'ai', 'nomod', 'link', 'off']);
}
...
function xaiTextFromResponse(data) {
  ...
  if (typeof message?.content === 'string' && stripThinking(message.content)) return stripThinking(message.content);
  if (Array.isArray(message?.content)) {
    const joined = message.content.map(part => part?.text || part?.content || '').filter(Boolean).join('\n');
    if (stripThinking(joined)) return stripThinking(joined);
  }
  // Старый completions-формат: текст лежит прямо в choices[0].text
  if (typeof choice?.text === 'string' && stripThinking(choice.text)) return stripThinking(choice.text);
...
// Убирает из ответа блоки размышлений вида <think>...</think>
...
async function askXaiText(mode, question, context = {}) {
  ...
  const timeout = setTimeout(() => controller.abort(), Number(env('XAI_TIMEOUT_MS', '40000')) || 40000);
...
function groupRulesText(groupType) {
  ...
  const rules = {
    ...
    link: [
      '🤝 СВЯЗЬ ГА — ГМ',
      'Беседа для прямой связи руководства администрации (ГА) и руководства модерации (ГМ).',
      '',
      '🎯 Для чего беседа',
      '— быстрая связь ГМ с любым из состава ГА по рабочим вопросам',
      '— обратная связь от ГА к руководству модерации',
      ...
      '— не игнорируем обращения: увидел вопрос к себе — ответь',
      '',
      '🔒 Всё, что обсуждается здесь, — строго конфиденциально.',
      'По вопросам работы беседы — пишите её создателю.',
    ],
    general: [
...
async function welcomeIfNeeded(peerId, message) {
  ...
  const hello = [
    ...
    `Ты в беседе: ${groupTypeTitle(groupType || 'general')}`,
    '━━━━━━━━━━━━━━',
    '',
...
async function handleGroupCommand(peerId, vkUserId, text) {
  ...
  if (create) {
    ...
    const title = `CHEREPOVETS | ${groupTypeTitle(requestedType)}`;
    let chatId;
    try {
