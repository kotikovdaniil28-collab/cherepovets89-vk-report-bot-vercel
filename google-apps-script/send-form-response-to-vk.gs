...
function normalizeExistingUrl_(value, type) {
  ...
  const formulaMatch = raw.match(/HYPERLINK\("([^"]+)"/i);
...
function repairStaffRow_(rowNumber) {
  ...
  sheet.getRange(n, 13).setFormula(`=IF(L${n}="";"";TODAY()-L${n})`);
  sheet.getRange(n, 15).setFormula(`=IF(N${n}="";"";TODAY()-N${n})`);

  return {
    ok: true,
    service: 'ch89-staff-sheet-repair-v16',
    sheetName: sheet.getName(),
    rowNumber: n,
    fixed: {
      vk: vkUrl || '',
