...
function fillStaffRow_(row) {
  ...
  sheet.getRange(rowNumber, 13).setFormula(daysFormula_(`L${rowNumber}`));
  sheet.getRange(rowNumber, 15).setFormula(daysFormula_(`N${rowNumber}`));

  return {
    ok: true,
    service: 'ch89-staff-sheet-fill-v25-application-to-staff',
    sheetName: sheet.getName(),
    rowNumber,
    spreadsheetUrl: SpreadsheetApp.getActiveSpreadsheet().getUrl(),
  };
...
function fillStaffRowFromApplication_(sheet, rowNumber) {
  ...
  return {
    ...result,
    service: 'ch89-application-to-staff-v25',
...
function repairStaffRow_(rowNumber) {
  ...
  const vkUrl = normalizeExistingUrl_(vkCell.getFormula() || vkCell.getDisplayValue(), 'vk');
  const forumUrl = normalizeExistingUrl_(forumCell.getFormula() || forumCell.getDisplayValue(), 'forum');
  const tgUrl = normalizeExistingUrl_(tgCell.getFormula() || tgCell.getDisplayValue(), 'telegram');

  if (vkUrl && vkUrl !== '—') vkCell.setFormula(hyperlinkFormula_(vkUrl, 'VK ↗'));
  if (forumUrl && forumUrl !== '—') forumCell.setFormula(hyperlinkFormula_(forumUrl, 'ФА ↗'));
  if (tgUrl && tgUrl !== '—') tgCell.setFormula(hyperlinkFormula_(tgUrl, 'TG ↗'));
  sheet.getRange(n, 13).setFormula(daysFormula_(`L${n}`));
  sheet.getRange(n, 15).setFormula(daysFormula_(`N${n}`));

  return {
    ok: true,
    service: 'ch89-staff-sheet-repair-v25-application-to-staff',
    sheetName: sheet.getName(),
    rowNumber: n,
    fixed: {
      vk: vkUrl || '',
...
function setApplicationVerdict_(sheet, rowNumber, verdict, reason, actor) {
  ...
  return {
    ok: true,
    service: 'ch89-google-sheets-verdict-v25',
    sheetName: sheet.getName(),
    rowNumber: n,
...
function doGet(e) {
  ...
  if (mode === 'staff_debug' || mode === 'staff_test') {
    ...
    return json_({
      ok: true,
      service: 'ch89-staff-sheet-v25-application-to-staff',
      staffFill: true,
  ...
  if (mode === 'debug' || mode === 'gsheet' || mode === 'diag') {
    return json_({ ok: true, service: 'ch89-google-sheets-v25-debug', ...debugInfo_(sheet) });
  }
  ...
  return json_({
    ok: true,
    service: 'ch89-google-sheets-pending-v25',
    sheetName: sheet.getName(),
    limit,
